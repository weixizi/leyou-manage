    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td class="justify-center layout px-0">
          ...
          <v-btn icon v-if="props.item.saleable" @click="soldOut(props.item.id)">下架</v-btn>
          <v-btn icon v-else @click="putaway(props.item.id)">上架</v-btn>
        </td>
      </template>
    </v-data-table>
    <!--弹出的对话框-->
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      deleteGoods(id){
        this.$http.delete("/item/spu/delete", {
          params: {
            id
          }
        }).then(resp => { // 这里使用箭头函数
      ...
      soldOut(id){
        this.$http.put("/item/spu/soldout",{
          params: {
            id
          }
        }).then(resp => {
      ...
      putaway(id){
        this.$http.put("/item/spu/putaway",{
          params: {
            id
          }
        }).then(resp => {
